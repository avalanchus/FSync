...
namespace FSyncManager
{
    public partial class HiddenMainForm : Form
    {
        ...
        private const string ServiceName = "FSyncService";
            ...
            params ServiceControllerStatus[] conditionStatuses)
        {
            if (conditionStatuses.Any(s => s == _service.Status))
            {
                ...
                {
                    notifyIcon.ShowBalloonTip(1000, "File Sync Manager", exception.Message, ToolTipIcon.Error);
                }
                RereshState();
            }
        }
